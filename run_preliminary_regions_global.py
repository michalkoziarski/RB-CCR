...
def evaluate_trial(trial):
    for minority_training_size in [-1, 10]:
        for dataset_name in datasets.names(small=True):
            RANDOM_STATE = 42
            RESULTS_PATH = Path(__file__).parents[0] / f'results_preliminary_regions_global_{minority_training_size}'
            ...
            resampler = ResamplingCV(
                CCRv5, classifier, seed=RANDOM_STATE, energy=energies,
                random_state=[RANDOM_STATE], gamma=['auto'],
