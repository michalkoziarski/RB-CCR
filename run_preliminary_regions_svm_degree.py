...
def evaluate_trial(trial):
    for minority_training_size in [-1]:
        for dataset_name in datasets.names():
            RANDOM_STATE = 42
            RESULTS_PATH = Path(__file__).parents[0] / f'results_preliminary_regions_svm_degree_{minority_training_size}'
            ...
            if classifier_name.startswith('svm'):
                degree = int(classifier_name[4:-1])
                classifier = SVC(random_state=RANDOM_STATE, degree=degree, kernel='poly')
            else:
